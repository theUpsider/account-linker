...
bot.on("message", async (message) => {
  ...
  // Staff check
  if (command.admin && !message.member.roles.cache.has(
        message.member.guild.roles.cache.find(
          (role) => role.name === settings.roles.staffrole
        ).id
      )) {
    return message.reply("You are not alled to execute this command!");
  }
  ...
  try {
    // check for reload!
    if(command.name === "reload" || "r")
      command.execute(message,args, bot) // requires special arg
    else
